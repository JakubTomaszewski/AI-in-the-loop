...
async def generate_prompts_all_classes(
    ...
    async def invoke_chain(class_name: str) -> str:
        ...
        # assert len(prompts) == args.num_synthetic_samples  # TODO: Add a mechanism for checking whether the number of prompts is correct. If not, retry the generation/generate more/cut the number of prompts
        return prompts
...
if __name__ == "__main__":
    ...
    while iteration_number < args.max_loop_iterations:
        ...
        class_information_history.append(class_information)

        logger.info(f"Class information history updated: {class_information_history}")

        logger.info("Generating prompt generation strategy")

        prompt_generation_strategy = strategy_generation_chain.invoke(
            {
                "class_information_history": class_information_history,
            }
        )
